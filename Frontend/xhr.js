...
function printChat(data) {
    ...
    // Отправляем на сервер запрос на выбор комнаты
    if (currentRoom !== '') {
        chatSocket.send(JSON.stringify({'usersendcommandroom': 'roomselect', 'newroom_id': id, 'oldroom_id': currentRoom}));
    } else {
        chatSocket.send(JSON.stringify({'usersendcommandroom': 'roomselect', 'newroom_id': id}));
    }

    currentRoom = id; // Обновляем currentRoom
}